...
import { removeTodo, setIsCompleted } from "../../redux/todoSlice";
import styles from "./TodoLists.module.scss";

type TodosListsTypes = {
  id: number;
  text: string;
  isCompleted: boolean;
};

const TodoLists: React.FC<TodosListsTypes> = ({ id, text, isCompleted }) => {
  ...
  return (
    <div className={classNames(styles.root, { isCompleted: isCompleted })}>
      <p>{text}</p>
      <div>
        <label htmlFor={"checbox" + id}>Complete</label>
        <input onChange={toggleComlete} type="checkbox" id={"checbox" + id} />
      </div>
      <svg
        onClick={deleteTodo}
